...
`meta-watch.sh` uses `rsync` to detect new files, runs `trec-auto-judge meta-evaluate` for multiple datasets, and syncs results back to a destination. It's designed to work with the `watch` command for periodic polling.
...
- **trec-auto-judge meta-evaluate**: Computes correlations between predicted and ground-truth leaderboards
